...
if rbversion >= '5':
    from rbtools.utils.mimetypes import (
        guess_mimetype,
        match_mimetype,
        parse_mimetype,
    )
...
def get_ticket_refs(text):
    ...
    parts = []
    lines = re.findall(
        r'(?im)^(?:issue|closes|fixes|see|bug|ticket):(.*)$', text
    )
    if not lines:
        return parts
    for line in lines:
...
class BaseDiffer(object):
    ...
    class DiffContent(object):
        ...
        def __init__(self, request_id, diff, base_commit_id, parent_diff=None):
...
class BaseReviewRequest(object):
    ...
    def __init__(self, root, repo, changesets, submitter, differ, web, topic):
        ...
        if r is not None and 'latest_diff' in r.links:
            self.diffset_id = r.get_latest_diff(
                only_links='', only_fields='id'
            ).id
    ...
    def _info_template(self):
        if self.request.created_with_history:
            return '```{author} [{branch}] [graft: {graft}] ```\n\n{desc}'
        else:
            return (
                '```{author} ({date}) [{node}] '
                '[{branch}] [graft: {graft}] '
        ...
        if self._info is None:
            ...
            for changeset in self._changesets:
                ...
                if merges:
                    ...
                    info += (
                        '# Touched %d file(s) by this merge '
                        'changeset\n' % len(files)
                    )
                    for entry in files:
                        info += '+ ' + entry + '\n'
    ...
    def modified(self):
        ...
        return (
            ...
            or self.request.summary != self.summary()
            or self._get_depends_on() != self._get_depends_on_updated()
            or self._modified_description()
            or not self._diff_up_to_date()
        )

    def prepare_close(self):
        if self._topic is None and self.request.commit_id != self.nodes(False):
            draft = self.request.get_or_create_draft(
                only_fields='', only_links='update'
            )
            draft.update(
    ...
    def _check_changesets(self):
        ...
        for changeset in self._changesets:
            ...
            if changeset.isMerge():
                raise HookError(
                    'Topic changeset is a merge: %s' % changeset.node()
                )
            ...
            if branch is None:
                ...
                raise HookError(
                    'Topic changesets uses multiple branches: '
    ...
    def _diff_up_to_date(self):
        ...
        if not self.exists() or self.diffset_id is None or 'UPDATE' in OPTIONS:
            return False

        e = self.request.extra_data
        return (
            'diff_hash' in e
            and self._get_hash(self.diffset_id) == e['diff_hash']
        )
    ...
    def _upload_diff_attachments(self, files):
        ...
        api = self.root.get_diff_file_attachments(
        ...
        supportedMimetypes = [
            ...
            for mimetype in SERVER.get_capability(
        ...
        def isKnown(filename, rev):
            for a in api.all_items:
                if (
                    a.repository_file_path == filename
                    and a.repository_revision == rev
                ):
                    return True
            return False
        ...
        def upload(id, filename, revision, source_file=False):
            ...
            def supported(content):
                ...
                valid = any(
                    match_mimetype(pattern, parse_mimetype(mime))
            ...
            size = self._differ.tool.get_file_size(
                filename=filename, revision=revision
            )

            if size > maxSize:
                log.info(
                    'File too large to upload: %s (%s)', filename, revision
                )
                return

            content = self._differ.tool.get_file_content(
                filename=filename, revision=revision
            )

            if supported(content):
                log.info(
                    'Upload binary "%s" in revision %s', filename, revision
                )
        ...
        for f in files:
            ...
            if (
                'parent_source_revision' in f.extra_data
                and f.source_revision != 'PRE-CREATION'
                and not isKnown(f.source_file, f.source_revision)
            ):
    ...
    def _update(self):
        ...
        extra_data = None
        draft = self.request.get_or_create_draft(
            only_fields='', only_links='update,draft_diffs'
        )
        binaries = []
        if not self._diff_up_to_date():
            diffs = draft.get_draft_diffs(
                only_links='upload_diff,draft_files', only_fields=''
            )
            ...
            if self.request.created_with_history:
                ...
                if len(self._changesets) > 1:
                    raise HookError(
                        'Cannot use ReviewRequest '
                        'with multiple changesets: %d' % self.request.id
                    )

                d = self.diff_info
                upload = diffs.upload_diff(
                    diff=d.getDiff(),
                    parent_diff=d.getParentDiff(),
                    base_commit_id=d.getBaseCommitId(),
                )
                if rbversion >= '1.0.2':
                    binaries = list(
                        upload.get_draft_files(binary=True).all_items
                    )

            # re-fetch diffset to get id
            diffs = draft.get_draft_diffs(
                only_links='draft_commits', only_fields='id'
            )
            extra_data = {'extra_data.diff_hash': self._get_hash(diffs[0].id)}

            binarySupportRB7 = rbversion >= '5' and hasCapability(
                'diffs', 'file_attachments'
            )
            if binarySupportRB7:
        ...
        for changeset in self._changesets:
            refs.extend(
                [six.text_type(x) for x in get_ticket_refs(changeset.desc())]
            )
        refs = list(set(refs))
        refs.sort()
        bugs = ','.join(refs)
    ...
    def _create(self):
        ...
        """
        c = self.root.get_review_requests(only_fields='', only_links='create')
    ...
    def _get_depends_on_updated(self):
        if self._depends_on_updated is None:
            ...
            if self._topic is None and changesetTopic is not None:
                ...
                requests = self._get_requests_from_summary(
    ...
    def _commit_id_data(self):
        ...
        if self._topic is None:
            s = self.summary()
            if (
                s.startswith('[maven-release-plugin]')
                or s.startswith('Added tag ')
                or s.startswith('Moved tag ')
                or s.startswith('Removed tag ')
            ):
    ...
    def _get_request(self):
        ...
        """
        fields = (
            'summary,approved,approval_failure,id,commit_id,depends_on,'
        ...
        if self._topic is None:
            ...
            reqs = self.root.get_review_requests(
                repository=self.repo,
                status='submitted',
                only_fields=fields,
                only_links=links,
            ...
            if len(reqs) > 0:
                raise HookError(
                    'Changeset "%s" is already known '
                    'in review request "%s"' % (self.nodes(), reqs[0].id)
                )
        ...
        count = len(reqs)
        if count == 0:
            reqs = self._get_requests_from_summary(
                fields, links, self.summary()
            )
            if len(reqs) == 1:
                return reqs[0]
            elif len(reqs) > 1:
                ...
                raise HookError(
                    'Found multiple review requests for summary '
                    '"%s": %s' % (self.summary(), ','.join(IDs))
                )
    ...
    def _generate_diff_info(self):
        self.diff_info = self._generate_diff(
            self._changesets[0].parent(),
        ...
        if self.request.created_with_history:
            if len(self._changesets) == 1:
                ...
                for changeset in self._changesets:
                    info = self._generate_diff(
                        changeset.parent(), changeset.node(), None
                    )
...
class MercurialReviewRequest(BaseReviewRequest):
    def __init__(self, root, repo, changeset, submitter, differ, web, topic):
        super(MercurialReviewRequest, self).__init__(
            root, repo, changeset, submitter, differ, web, topic
        )
    ...
    def _update_attachments(self, uploaded):
        ...
        a = self.request.get_file_attachments(
            only_fields='caption,attachment_history_id', only_links='delete'
        )
        hashes = {}
        existing = {}
        ...
        for entry in set(adds + mods):
            if self.regexUpload.match(entry) and entry not in uploaded:
...
class GitReviewRequest(BaseReviewRequest):
    def __init__(self, root, repo, changeset, submitter, differ, web, topic):
        super(GitReviewRequest, self).__init__(
            root, repo, changeset, submitter, differ, web, topic
        )
    ...
    def approved(self):
        ...
        if approved:
            for rev in self._changesets:
                if rev.hasDangling():
                    self._failure = (
                        'The merge has dangling changesets: %s'
    ...
    def _info_template(self):
        if self.request.created_with_history:
            ...
            return '```{author} ({date}) [{node}] [{branch}]```\n\n{desc}'
    ...
    def _generate_diff(self, parent, node, base):
        ...
        return super(GitReviewRequest, self)._generate_diff(parent, node, base)
...
class BaseRevision(object):
    ...
    def topic(self):
        if self._topic is None:
            matches = re.findall(
                r'^topic:([a-zA-Z0-9/_ .-]+)', self.desc(), re.MULTILINE
            )
            if len(matches) > 0:
...
class MercurialRevision(BaseRevision):
    ...
    def fetch(revset, fetch_base=False, debug=True):
        ...
        changes = execute(cmd, with_errors=False, return_errors=False)
    ...
    def files(self, template='{files|json}'):
        return json.loads(
            execute([HG, 'log', '-r', self.node(), '--template', template])
        )

    def file(self, filename):
        return execute(
            [HG, 'cat', '-r', self.node(), filename],
            with_errors=False,
...
class GitRevision(BaseRevision):
    ...
    def fetch_known_refs(rev):
        ...
        for line in output.splitlines():
            ...
            if values[1] in ('commit', 'tag') and (
                'refs/heads/' in values[2] or 'refs/tags/' in values[2]
            ):
    ...
    def hasDangling(self):
        if self._has_dangling is None:
            self._has_dangling = (
                self.isMerge()
                and len(GitRevision.fetch_known_refs(self.parent(1))) == 0
            )
    ...
    def merges(self, nodes=None):
        ...
        """
        if self.isMerge() and self._merges is None:
            self._merges = GitRevision.fetch(
                self.node(), self.parent(), skipKnown=False
            )
...
class BaseHook(object):
    def __init__(self, name, review_request_class, review_differ_class):
        ...
        if 'KALLITHEA_EXTRAS' in e:
            ...
            'HEPTAPOD_USERINFO_USERNAME' in e
            and 'HEPTAPOD_PROJECT_PATH' in e
            and 'HEPTAPOD_PROJECT_NAMESPACE_FULL_PATH' in e
        ):
            ...
            'GITEA_REPO_NAME' in e
            and 'GITEA_PUSHER_NAME' in e
            and 'GITEA_REPO_USER_NAME' in e
        ):
            self.submitter = e['GITEA_PUSHER_NAME']
            self.repo_name = (
                e['GITEA_REPO_USER_NAME'] + '/' + e['GITEA_REPO_NAME']
            )
        elif (
            'GOGS_REPO_NAME' in e
            and 'GOGS_AUTH_USER_NAME' in e
            and 'GOGS_REPO_OWNER_NAME' in e
        ):
            self.submitter = e['GOGS_AUTH_USER_NAME']
            self.repo_name = (
                e['GOGS_REPO_OWNER_NAME'] + '/' + e['GOGS_REPO_NAME']
            )
    ...
    def _set_repo_id(self):
        ...
        repos = self.root.get_repositories(
            name=self.repo_name,
            tool=self.name,
            only_fields=fields,
            only_links='',
        ...
        if repos.num_items < 1:
            ...
            if repos.num_items < 1:
                raise HookError(
                    'Could not open Review Board repository:'
                    '\n%s\n'
    ...
    def _init_rbtools(self, cmd):
        ...
        try:
            ...
            log.error(
                'Cannot fetch data from RB (%s). Is ALLOWED_HOST correct?',
                server_url,
            )
            raise

        session = get_authenticated_session(
            api_client, root, auth_required=True, num_retries=0
        )
        if session is None or not session.authenticated:
            raise HookError(
                'Please add an USERNAME and a PASSWORD or '
    ...
    def _set_root(self):
        ...
        try:
            ...
            log.info(
                'Trying .reviewboardrc (RBTOOLS_CONFIG_PATH) file "'
                'in "%s" and "%s"',
                os.environ.get('HOME'),
    ...
    def _check_duplicate(self, req, revreqs):
        ...
        return any(
            not r.skippable()
            and (r.summary() == req.summary() or r.commit_id == req.commit_id)
            for r in revreqs
        )
            ...
            if topic:
                if currentTopic is None:
                    ...
                    if topic in topicchanges:
                        raise HookError(
                            'Topic is out of order: %s' % currentTopic
                        )
                    currentTopic = topic
    ...
    def _handle_changeset_list_process(self, changesets):
        ...
        for topic in topicchanges:
            log.info(
                "Use topic '%s' with %d changeset(s)",
                topic,
                len(topicchanges[topic]),
            )
            self._handle_changeset_list_process_request(
                topicchanges[topic], revreqs, topic
            )

        return self._handle_approved_review_requests(revreqs)

    def _handle_changeset_list_process_request(
        self, changesets, revreqs, topic=None
    ):
        request = self.review_request_class(
            self.root,
            self.repo_id,
            changesets,
            self.submitter,
        ...
        if self._check_duplicate(request, revreqs):
            log.info(
                'Ignoring changeset(s) (%s) as it has a '
    ...
    def _log_push_info(self, changeset):
        log.info(
            'If you want to push the already approved '
    ...
    def _handle_review_request(self, request):
        ...
        if request.exists():
            if request.modified():
                if request.sync():
                    log.info(
                        'Updated review request (%d) for changeset(s): %s',
                        request.id(),
                        request.nodes(),
                    )
                else:
                    log.info(
                        'Skipped update of review request (%d) for '
                        'changeset(s): %s',
                        request.id(),
                        request.nodes(),
                    )
                ...
                if request.approved():
                    log.info(
                        'Found approved review request (%d) for '
                        'changeset(s): %s',
                        request.id(),
                        request.nodes(),
                    )
                    ...
                    log.info(
                        'Found unchanged review request (%d) for '
                        'changeset(s): %s | %s',
                        request.id(),
                        request.nodes(),
                        request.failure(),
            ...
            if request.sync():
                log.info(
                    'Created review request (%d) for changeset(s): %s',
                    request.id(),
                    request.nodes(),
                )
            else:
                log.info(
                    'Skipped creation of review request for changeset(s): %s',
                    request.nodes(),
                )
    ...
    def push_to_reviewboard(self, pushinfo):
        ...
        log.info(
            'Push as user "%s" to "%s"...', self.submitter, self.repo_name
        )
...
class MercurialHook(BaseHook):
    ...
    def __init__(self):
        super(MercurialHook, self).__init__(
            'Mercurial', MercurialReviewRequest, MercurialDiffer
        )
    ...
    def _is_multi_head(self):
        heads = MercurialRevision.fetch(
            'head() and not closed()', False, False
        )
        if heads is None:
...
class GitHook(BaseHook):
    ...
    def __init__(self):
        super(GitHook, self).__init__('Git', GitReviewRequest, GitDiffer)
        if self.repo_name is None:
    ...
    def _check_signatures(self, changesets):
        ...
        for changeset in changesets:
            if (
                changeset.signTrust() not in hookSignTrust
                or changeset.signVerify() != 'G'
            ):
                log.info(
                    'Signature of changeset (%s) invalid. '
    ...
    def _handle_changeset_list_process(self, changesets):
        ...
        if len(changesets) > 1:
            ...
            if len(mergeIncoming) > 0:
                ...
                def isDangling(rev):
                    return (
                        rev.branch() in mergeIncoming
                        and rev.node() in mergeIncoming[rev.branch()]
                    )
    ...
    def _set_repo_id(self):
        ...
        if (
            'hosting_url' in d
            and 'gitlab_group_name' in d
            and 'gitlab_group_repo_name' in d
        ):
            self.web = (
                d['hosting_url']
                + '/'
    ...
    def _log_push_info(self, changeset):
        ...
        log.info(
            'git push origin %s:%s', changeset.node(True), changeset.branch()
        )
...
def get_logging_level():
    if 'DEBUG' in OPTIONS and OPTIONS['DEBUG'].lower() in (
        'true',
        'on',
        '1',
        '',
...
def set_options():
    ...
    if GIT_OPT_COUNT in os.environ:
        ...
        for option in range(0, optionCount):
            ...
            OPTIONS[value[0].upper()] = value[1] if len(value) > 1 else ''
    else:
...
def hook(stdin=None):
    try:
        set_globals()
        logging.basicConfig(
            format='%(levelname)s: %(message)s', level=get_logging_level()
        )
        environment_check()
