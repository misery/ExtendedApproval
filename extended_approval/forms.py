...
class ExtendedApprovalSettingsForm(SettingsForm):
    ...
    enable_target_shipits = BooleanField(
        ...
        help_text=(
            'Do not accept ShipIts if the user is not in review group/people.'
        ),
    )
    ...
    forbidden_approve_prefixes = CharField(
        required=False,
        label='Non approvable commit message (summary) prefixes',
    ...
    forbidden_approve_suffixes = CharField(
        required=False,
        label='Non approvable commit message (summary) suffixes',
